...
class SimpleGenerator:
    ...
    def get_item(self, rand_key, group="train", shift=0) -> (jnp.ndarray, jnp.ndarray):
        ...
        for obj in self.space[group]:
            ...
            ns_index_center, ns_xyz_center = obj.sampling_eachN_from_mesh(
                count=self.sample_from_each_model, step=self.step, shift=shift
            )
            ...
            for ind, xyz in enumerate(ns_xyz_center):
                _, ns_sdt = obj.surface_xyz2localsdt(
                    xyz, spacing=self.spacing, scale=self.scale
                )
    ...
    def viz_item(
        self, group="test", patient="patient069"
        ...
        ns_index_center, ns_xyz_center = obj.sampling_eachN_from_mesh(
            count=num_of_points, step=1, shift=0
        )
...
if __name__ == "__main__":
    ...
    # NN initialization
    task = SurfaceSegmentation(
        spacing=SHAPE, conv_kernel=32, conv_depth=4, num_classes=3, batch_size=128
    )
    ...
    for epoch in viz.iter(EPOCHS):
        ...
        loss, accuracy, params, rng, opt_state = train_step(
            params, rng, opt_state, X, Y
        )
        ...
        if viz.is_print_on_epoch(epoch):
            ...
            viz.save_mesh(
                "patient069",
                save_mesh,
                {"pred_class": onp.array(viz_Y_pred), "class": onp.array(viz_Y)},
            )

            X, Y = sg.get_item(
                subkey, group="train", shift=4 * int(epoch / viz.print_on_each_epoch)
            )
            print("Class balance: ", jnp.sum(Y == 0), jnp.sum(Y == 1), jnp.sum(Y == 2))
