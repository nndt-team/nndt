...
class MethodSetTestCase(unittest.TestCase):
    ...
    def test_transform_ident(self):
        ...
        self.assertEqual(7.0, jnp.array(space.patient009.ns.xyz_ps2ns(ps_grid)).min())
        self.assertEqual(183.0, jnp.array(space.patient009.ns.xyz_ps2ns(ps_grid)).max())
        self.assertEqual(7.0, jnp.array(space.patient009.ns.xyz_ns2ps(ns_grid)).min())
        self.assertEqual(183.0, jnp.array(space.patient009.ns.xyz_ns2ps(ns_grid)).max())
    ...
    def test_transform_to_cube(self):
        ...
        self.assertEqual(-1., float(jnp.array(space.patient009.ns.xyz_ps2ns(ps_grid)).min()))
        self.assertEqual(1., float(jnp.array(space.patient009.ns.xyz_ps2ns(ps_grid)).max()))
        self.assertEqual(7.0, float(jnp.array(space.patient009.ns.xyz_ns2ps(ns_grid)).min()))
        self.assertEqual(183.0, float(jnp.array(space.patient009.ns.xyz_ns2ps(ns_grid)).max()))
    ...
    def test_transform_shift_and_scale(self):
        ...
        self.assertAlmostEqual(-0.7699, float(jnp.array(space.patient009.ns.xyz_ps2ns(ps_grid)).min()), places=2)
        self.assertAlmostEqual(0.7699, float(jnp.array(space.patient009.ns.xyz_ps2ns(ps_grid)).max()), places=2)
        self.assertAlmostEqual(7.0, float(jnp.array(space.patient009.ns.xyz_ns2ps(ns_grid)).min()), places=2)
        self.assertAlmostEqual(183.0, float(jnp.array(space.patient009.ns.xyz_ns2ps(ns_grid)).max()), places=2)
