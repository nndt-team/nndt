...
class MyTestCase:  # (BaseTestCase): # (unittest.TestCase):
    ...
    def obj_only(self, array, tpl=(FileSource,)):
        return [x for x in array if isinstance(x, tpl)]
    ...
    def test_OLD_function_load_data(self):
        space = load_from_file_lists(self.name_list, self.mesh_list, self.sdt_list)
        ...
        self.assertEqual(2, len(self.obj_only(obj.children)))

    def test_OLD_ExtendedNodeMixin(self):
        space = load_from_file_lists(self.name_list, self.mesh_list, self.sdt_list)

        self.assertTrue(space.is_root)
        self.assertEqual(5, len(self.obj_only(space["default/"].children, (Object3D,))))
        self.assertEqual(2, len(self.obj_only(space["default/patient009"].children)))
        print(space.explore())
    ...
    def test_load_meshes(self):
        ...
        self.assertTrue(onp.allclose((((-0.9467665481567383, -0.9056295013427734, -0.9056295013427734),
                                     ...
                                     space["default/patient009/ns"].bbox, rtol=0., atol=1.e-2
                                     ))
        self.assertTrue(onp.allclose(((-0.9467665481567383, -0.9056295013427734, -0.9056295013427734),
                                      (0.9467665481567383, 0.9056295013427734, 0.9056295013427734)),
                                     space["default/patient009"].bbox, rtol=0., atol=1.e-2
                                     ))
        self.assertTrue(onp.allclose(((-0.9522853851318359, -0.96994, -0.96994),
                                      (0.9522853851318359, 0.9699400000000002, 0.9699400000000002)),
                                     space["default"].bbox, rtol=0., atol=1.e-2
                                     ))
        self.assertTrue(onp.allclose(((-0.9522853851318359, -0.96994, -0.96994),
                                      (0.9522853851318359, 0.9699400000000002, 0.9699400000000002)),
                                     space.bbox, rtol=0., atol=1.e-2
                                     ))
    ...
    def test_sampling_from_bbox(self):
        ...
        sampling = space["default/patient009/ns/sampling_grid"](spacing=(2, 2, 2))
    ...
    def test_test_train_split(self):
        space = load_from_file_lists(self.name_list, self.mesh_list, self.sdt_list, test_size=0.5)
        space.initialization('shift_and_scale')
        print(space.explore())
