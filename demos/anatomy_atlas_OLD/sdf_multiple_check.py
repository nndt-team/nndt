...
LOAD_DATA = './sdf_multiple_files/sdf_model.pkl'
EXP_NAME = 'sdf_multiple_files_check'
...
if __name__ == '__main__':
    ...
    for c in tqdm(bary):
        c_ = jnp.insert(c, slice(2, 4), jnp.array([0., 0.]))
        ...
        viz.sdf_to_obj(f"SDF_{c[0]}_{c[1]}_{c[2]}", predict_sdf, level=LEVEL_SHIFT)

    bary = barycentric_grid(order = ((-1, -2), (-1, 2), (1, -2), (1, 2)),
                              spacing = (0, 5, 5),
    ...
    for c in tqdm(bary):
        ...
        viz.sdf_to_obj(f"SDF2_{c[0]}_{c[1]}_{c[2]}_{c[3]}", predict_sdf, level=LEVEL_SHIFT)